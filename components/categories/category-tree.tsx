  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm h-full">
      <div className="flex items-center justify-between mb-8">
        ...
        <button className="text-xs font-semibold text-[#6338f6] hover:underline">Expand All</button>
      </div>
          ...
          <div key={node.id} className="space-y-2">
            <div 
              className={cn(
                ...
                expanded.includes(node.id) ? "bg-[#f8f7ff]" : "hover:bg-gray-50"
              )}
            ...
            {node.children && expanded.includes(node.id) && (
              <div className="ml-12 space-y-2 border-l border-gray-100 pl-4">
                {node.children.map((child) => (
                  <div key={child.id} className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg cursor-pointer">
